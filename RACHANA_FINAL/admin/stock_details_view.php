...
<button type="button" class="btn" style="float:right; margin-right:5%; background-color:#1A5F60; color:white;" onclick="window.location.href='stock_details_form.php'">NEW</button>
...
<tbody>
<?php 
$sql=$conn->prepare("SELECT * FROM stock_details,product_details WHERE stock_details.pd_id=product_details.pd_id ORDER BY product_details.pd_id DESC");
$sql->execute();
...
<td>
<a class="btn  btn-danger" href="stock_details_delete.php?id=<?php echo $row['sd_id'];?>" onclick="return confirm('Do You Really Want Delete ?')">DELETE</a>
</td>
</tr>
<?php
}
