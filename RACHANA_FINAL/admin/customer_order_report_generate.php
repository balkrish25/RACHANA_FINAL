...
$pdf->SetWidths(array(20,34,28,40,40,30,30,25,30));
$pdf->RowBorder(array('SL NO.','DATE','ORDER NO','CUSTOMER NAME','PAYMENT MODE','SUB TOTAL','DELIVERY CHARGE','TAX','GRAND TOTAL'),8,'C');
   ...
   while($row=$result->fetch_assoc())
    {       
       ...
       if($json_array["shipping_details"]["payment_mode"] == "COD")
        ...
        $pdf->RowBorder(array($slno++,$date,$row["order_no"],$row["cu_name"]." ".$row["cu_contact"],$paymentmode,currencyFormat($json_array["amount_details"]["total"]),currencyFormat($json_array["amount_details"]["discount_charges"]),currencyFormat($json_array["amount_details"]["tax"]),currencyFormat($json_array["amount_details"]["grand_total"])),8,'C');
    } 
