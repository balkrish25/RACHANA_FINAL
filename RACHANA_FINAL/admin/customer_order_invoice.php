<?php
   ...
   $pdf->SetWidths(array(14,60,30,25,30,30));

   $sl=0;
    foreach($json_array["product_details"] as $row) 
	{                               
	   $sl=$sl+1;
	   $pdf->RowBorder(array($sl,$row['name'],currencyFormat($row['price']),currencyFormat($row['discount_percent']),$row['qty'],currencyFormat($row['totalamount'])),8,'C');
	}
   
   $pdf->Cell(159,10,'Total',1,0,"R");
   $pdf->Cell(30,10,currencyFormat($json_array["amount_details"]["total"]),1,1,"C");
	$pdf->Cell(159,10,'Tax',1,0,"R");
   $pdf->Cell(30,10,currencyFormat($json_array["amount_details"]["tax"]),1,1,"C");
	$pdf->Cell(159,10,'Discount Charges',1,0,"R");
   $pdf->Cell(30,10,currencyFormat($json_array["amount_details"]["discount_charges"]),1,1,"C");
$pdf->Cell(159,10,'Grand Total',1,0,"R");
   $pdf->Cell(30,10,currencyFormat($json_array["amount_details"]["grand_total"]),1,1,"C");
